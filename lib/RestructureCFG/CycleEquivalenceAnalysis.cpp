...
void CycleEquivalenceAnalysis<GraphT, GT>::insertEdge(BlockEdgeDescriptor E,
  ...
  using NakedEdgeDescriptor = CycleEquivalenceClass<NodeT>::EdgeDescriptor;
  NakedEdgeDescriptor NakedEdge;
  if (not IsInverted) {
    NakedEdge = NakedEdgeDescriptor({ std::get<0>(E)->getBlock(),
                                      std::get<1>(E)->getBlock(),
                                      Label->succNum() });
  } else {
    NakedEdge = NakedEdgeDescriptor({ std::get<1>(E)->getBlock(),
...
void CycleEquivalenceAnalysis<GraphT, GT>::insertCappingBackedges(BlockGraph
  ...
  // `BB`. Please note that for higher nodes, the DFSNum is smaller. This use of
  // the highest term is made to be coherent with the terminology used in the
  // paper describing the cycle equivalence algorithm. It is highly suggested
  // to read it in order to get confident with the terminology.
  std::map<BlockNode *, size_t> HighestReachedDFSNum;

  // Keep a mapping between the `DFSNum` and the corresponding node
  std::map<size_t, BlockNode *> DFSNumNodeMap;
  ...
  for (BlockNode *BB : PostOrder) {
    ...
    // 1) Compute the highest reached node from a backedge starting from the
    //    current `Block`
    ...
    for (const auto &[BackwardEdge, _, __] : getBackwardEdges(BB)) {
      ...
      HI0 = std::min(HI0, Target->getDFSNum());

      // We also populate the mapping between the `DFSNum` and the node
      DFSNumNodeMap[Target->getDFSNum()] = Target;
    }

    // 2) Compute the highest reached node for any child of the current `Block`
    //    (represented by the HI1 index), and the `HI1Child`, which is the child
    //    from which the subtree that reaches the `HI1` node originates
    ...
    for (const auto &[ForwardEdge, _, __] : getForwardEdges(BB)) {
      ...
      size_t NewMin = std::min(HI1, HighestReachedDFSNum.at(Target));
      if (NewMin < HI1) {
        HI1Child = Target;
      }
      HI1 = NewMin;
    ...
    HighestReachedDFSNum[BB] = std::min(HI0, HI1);

    // 4) This is the highest reached node for any child of the current `Block`,
    //    excluding the one which reaches the highest target
    size_t HI2 = SizeTMaxValue;
    for (const auto &[ForwardEdge, _, __] : getForwardEdges(BB)) {
      BlockNode *Target = std::get<1>(ForwardEdge);
      if (Target != HI1Child) {
        HI2 = std::min(HI2, HighestReachedDFSNum.at(Target));
      }
    }
    ...
    // node.
    if (HI2 < HI0) {
      BlockNode *HI2Node = DFSNumNodeMap.at(HI2);
